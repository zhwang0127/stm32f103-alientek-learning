...
/**
 * @brief       在OLED上显示一个16x16中文字符（PCtoLCD2002逐列式字库）
 * @note        字库数据格式: 阴码 + 逐列式 + 顺向 + C51格式
 *              每列2字节(上行8点+下行8点), 共16列, 每字32字节
 *              字库数据在 chinese_font.h 的 CHINESE_16X16 数组中
 * @param       x    : 起始横坐标 0~127
 * @param       y    : 起始纵坐标 0~63
 * @param       index: 中文字库索引号（见 chinese_font.h 中的 CN_XXX 宏）
 * @retval      无
 */
void atk_oled_show_chinese(uint8_t x, uint8_t y, uint8_t index)
{
    uint8_t col, bit;
    const uint8_t *font;

    if (index >= CN_CHAR_COUNT) return;             /* 越界保护 */
    if (x > 111 || y > 48) return;                  /* 超出128×64范围 (16px字符y最大48) */

    font = CHINESE_16X16[index];                     /* 指向该字符的字模数据 */

    for (col = 0; col < 16; col++)                   /* 16列 */
    {
        uint8_t byte_upper = font[col * 2];          /* 该列的上8行 (bit7=row0, bit0=row7) */
        uint8_t byte_lower = font[col * 2 + 1];      /* 该列的下8行 (bit7=row8, bit0=row15) */

        for (bit = 0; bit < 8; bit++)
        {
            if (byte_upper & (0x80 >> bit))
                atk_oled_draw_point(x + col, y + bit, 1);       /* 上半部分 */
            if (byte_lower & (0x80 >> bit))
                atk_oled_draw_point(x + col, y + 8 + bit, 1);   /* 下半部分 */
        }
    }
}

/**
 * @brief       在OLED上显示中文字符串（16x16逐列式字库）
 * @param       x      : 起始横坐标
 * @param       y      : 起始纵坐标
 * @param       indices: 字库索引数组
 * @param       len    : 字符个数
 * @retval      无
 */
void atk_oled_show_chinese_str(uint8_t x, uint8_t y, const uint8_t *indices, uint8_t len)
{
    uint8_t i;

    for (i = 0; i < len; i++)
    {
        atk_oled_show_chinese(x + i * 16, y, indices[i]);   /* 每个汉字占16像素宽 */
    }
}
